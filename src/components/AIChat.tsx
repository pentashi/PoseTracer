...
const AIChat = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('❌ Failed to get AI reply', await res.text());
        setAiTyping(false);
        return;
      }
  ...
  const handleQuickPrompt = (text: string) => setInputMessage(text);
    ...
    <div className="min-h-screen bg-gradient-to-b from-cyber-darker to-background p-4 flex flex-col">
      ...
      <div className="flex-1 space-y-4 mb-6 overflow-y-auto max-h-[60vh]">
        {chatHistory.map((msg, idx) => (
          <Card
            key={idx}
            className={`p-4 ${msg.type === 'ai' ? 'cyber-card ml-0 mr-12' : 'bg-neon-purple/20 border-neon-purple ml-12 mr-0'}`}
          >
                ...
                {msg.insights?.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {msg.insights.map((insight, i) => (
                      <Badge key={i} className="bg-neon-blue/20 text-neon-blue border-neon-blue/30">{insight}</Badge>
                    ))}
                  </div>
                )}
              </div>
        ...
        {aiTyping && <div className="text-neon-blue italic text-sm flex items-center">AI Coach is typing...</div>}
        <div ref={chatEndRef} />
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {quickPrompts.map((prompt, idx) => (
            <Button key={idx} variant="ghost_cyber" size="sm" className="text-xs" onClick={() => handleQuickPrompt(prompt.text)}>
