          ...
          return (
            <Button
              ...
              className={`
                ...
                ${isActive ? 'ai-glow' : ''}
                xs:min-w-0 xs:px-1 xs:text-[9px]
              `}
              onClick={() => navigate(item.path)}
            >
              <Icon
                className={`h-4 w-4 ${isActive ? 'text-cyber-dark' : 'text-neon-purple'} xs:h-3 xs:w-3`}
              />
              <span className={`
                text-xs ${isActive ? 'text-cyber-dark font-semibold' : 'text-muted-foreground'}
                xs:text-[9px]
              `}>
                {item.label}
              </span>
